...
const Service = ({
  ...
  title,
  description
}: ServiceProps) => <div className="flex flex-col items-center p-6 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow">
  ...
  return <div className="py-16 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">Experience Premium Hospitality Services Designed for your Comfort and Satisfaction at Grand Ambar Palace Hotel & Suites!</p>
          <div className="w-20 h-1 bg-accent-red mx-auto mt-4"></div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => <Service key={index} icon={service.icon} title={service.title} description={service.description} />)}
        </div>
      </div>
    </div>;
};
