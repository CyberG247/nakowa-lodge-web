...
const HeroSlider = () => {
  ...
  return <div className="relative w-full h-[70vh] overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center text-white px-4">
              ...
              <div className="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-4">
                <Link to="/booking">
                  <Button className="booking-btn bg-accent-red hover:bg-navy hover:text-white px-8 py-6 text-lg">
                    Book Now
                  </Button>
                </Link>
                <Link to="/rooms">
      ...
      {/* Navigation Arrows */}
      <button className="absolute left-4 top-1/2 transform -translate-y-1/2 p-2 rounded-full bg-black bg-opacity-30 text-white hover:bg-opacity-50 transition-all z-10" onClick={prevSlide} aria-label="Previous slide">
        <ChevronLeft size={24} />
      </button>
      
      <button className="absolute right-4 top-1/2 transform -translate-y-1/2 p-2 rounded-full bg-black bg-opacity-30 text-white hover:bg-opacity-50 transition-all z-10" onClick={nextSlide} aria-label="Next slide">
        <ChevronRight size={24} />
      </button>
      
      {/* Dot Indicators */}
      <div className="absolute bottom-5 left-0 right-0 flex justify-center space-x-2">
        {slides.map((_, index) => <button key={index} className={`h-2 w-2 rounded-full ${index === currentSlide ? 'bg-white w-4' : 'bg-white bg-opacity-50'} transition-all`} onClick={() => setCurrentSlide(index)} aria-label={`Go to slide ${index + 1}`} />)}
      </div>
    </div>;
};
